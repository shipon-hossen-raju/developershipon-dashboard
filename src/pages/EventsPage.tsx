...
import type { EventFormData, PortfolioEvent } from "../types";
...
export default function EventsPage() {
  ...
  const handleSave = async () => {
    if (!form.title || !form.organizer) {
      toast.error("Title and organizer are required");
      return;
    }
    try {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Events"
        desc={`${items.length} events · ${items.filter((e) => e.highlight).length} featured`}
        action={
      ...
      <DataTable
        ...
        keyField="id"
        searchable
        searchKeys={["title", "organizer", "role"]}
        emptyState={
          <EmptyState
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Date" hint='e.g. "March 2024"'>
              <Input
                value={form.date}
